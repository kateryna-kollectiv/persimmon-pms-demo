...
const App = () => {
  ...
  const [tasksInboxOpen, setTasksInboxOpen] = React.useState(false);
  ...
  const titles = {
    huddle: { t: "Good morning, Sasha", s: <><b>39 total patient visits</b><span className="dot" /><span style={{ color: "#5A7A2F" }}>2 waiting</span><span className="dot" /><span style={{ color: "var(--persimmon)" }}>2 in chair</span></> },
    schedule: { t: "Schedule", s: <><b>39 appointments scheduled</b><span className="dot" /><span style={{ color: "#5A7A2F" }}>2 waiting</span><span className="dot" /><span style={{ color: "var(--persimmon)" }}>2 in chair</span></> },
    patients: { t: "Patients", s: <><b>432 active patients</b><span className="dot" /><span>2 recall due</span></> }
  };
  ...
  return (
    <div className="app">
      ...
      {!openPatient && <>
        <TopBar title={titles[view].t} sub={titles[view].s} onTasksClick={() => setTasksInboxOpen(true)} />
        <SubToolbar>
      ...
      view === "huddle" ? <Huddle onOpenTask={(id) => setOpenTask(id)} onGoSchedule={() => setView("schedule")} /> :
      ...
      {tasksInboxOpen &&
      <TasksInbox
        onClose={() => setTasksInboxOpen(false)}
        onOpenPatient={(id) => {setTasksInboxOpen(false);setOpenPatient(id);setView("patients");}}
        onComplete={(id) => {setTasksInboxOpen(false);showToast("Task marked complete");}} />
